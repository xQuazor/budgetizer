...
AudioPluginAudioProcessor::createParameters() {
    ...
    params.push_back(std::make_unique<juce::AudioParameterBool>("widen", "Widen", true));
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        "sampleReductionRate", "Sample Reduction Rate", 0, 22, 4));

    // Radio Layout
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        "chaos", "chaos", 0.0f, 1.0f, 0.25f));
    params.push_back(std::make_unique<juce::AudioParameterFloat>(
        "interval", "Interval", 0.0f, 2000.0f, 500.0f));

    // Master Knob
    params.push_back(std::make_unique<juce::AudioParameterBool>("useAudioInput", "useAudioInput", true));
...
void AudioPluginAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
    ...
    const bool noiseEnabled = *apvts.getRawParameterValue("widen");
    ...
    multipath.setBaseDelay(3.0f);
    multipath.setMix(1.0f);
    preEmphasis.setAmount(1.0f);

    radioEffect.setTriangleDepth(chaos);
    radioEffect.setDropoutAmount(chaos);

    radioEffect.setNoiseLevel(0.25f);
    ...
    for (int sample = 0; sample < numSamples; ++sample) {
        ...
        for (int ch = 0; ch < numChannels; ++ch) {
            float mod = abs(complexLFO.returnModulation()) * 2500.0f;
            ...
            if (radio) {
                if (ch == 0) {
                    ++logSampleCounter;
                    if (logSampleCounter >= static_cast<int>(getSampleRate() * 0.2)) {
                        DBG("mod: " << mod);
                        logSampleCounter = 0;
                    }
                }
            ...
            if (chaos > 0 || interval > 0) {
                radioEffect.setFormantShift(float(chaos / 100.0f));
                radioEffect.setCurrentFrequency(mod);
